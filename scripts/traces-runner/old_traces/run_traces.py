...
def main():
	...
	while True:
		...
		proc = subprocess.Popen('mm-delay ' + str(MM_DELAY) + 
				  ' mm-link --uplink-log=./results/cubic_short_uplink --downlink-log=./results/cubic_short_downlink ' + trace_path + f1 + ' ' + '../cooked_traces/Verizon-LTE-short.down  /usr/bin/python ' + RUN_SCRIPT + ' ' + ip + ' ' + abr_algo + ' ' + str(RUN_TIME) + ' ' + process_id + ' ' + f2 + ' ' + str(sleep_time), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
