...
def main():
	...
	while True:
		...
		proc = subprocess.Popen('mm-delay ' + str(MM_DELAY) + 
				  ' mm-loss downlink 0.05 mm-link --uplink-log=./results/cubic_att_uplink --downlink-log=./results/cubic_att_downlink ' + trace_path + f1 + ' ' + '../cooked_traces/ATT-LTE-driving.down /usr/bin/python ' + RUN_SCRIPT + ' ' + ip + ' ' + abr_algo + ' ' + str(RUN_TIME) + ' ' + process_id + ' ' + f2 + ' ' + str(sleep_time), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
