...
rl_server_address = "http://localhost:8333/"
MTU_SIZE = 1500
TOTAL_CHUNK_NUM = 48
# statistics needed by mpc_server.py on http://localhost:8333/
# lastquality: quality of last chunk, default is 1
lastquality = 1
# lastRequest: index of last chunk
lastRequest = 0
buffer = 0.0
RebufferTime = 0
lastChunkStartTime = 0
# data = {u'lastRequest': 48, u'lastquality': 5, u'lastChunkStartTime': 1576767764502, u'buffer': 8.302235, u'bufferAdjusted': 3.752234999999999, u'nextChunkSize': [2956927, 2160877, 1520281, 933276, 617681, 382355, 252010, 155432, 102175], u'bandwidthEst': 717.368080547366, u'RebufferTime': 7706, u'lastChunkFinishTime': 1576767765926, u'lastChunkSize': 139857}\

currIndex = 0 # current index in mahimahi

def loadTrace(tracePath):
    trace = []
...
def getDownloadTime(chunkSize, trace):
    global currIndex
    initialTime = trace[currIndex]
    downloadTime = 0
    while chunkSize > 0:
        chunkSize -= MTU_SIZE
        downloadTime = trace[currIndex] - initialTime
        currIndex += 1
        if currIndex >= len(trace):
            currIndex = 0
    return downloadTime

trace = loadTrace("../cooked_traces/fcc0_.down")

# important notes for unit:
# buffer is in s
# lastChunkFinishTime, lastChunkStartTime, RebufferTime are in ms
while lastRequest <= TOTAL_CHUNK_NUM:
    # get statistics
    lastChunkSize = get_chunk_size(lastquality, lastRequest)
    downloadTime = getDownloadTime(lastChunkSize, trace)
    lastChunkFinishTime = lastChunkStartTime + downloadTime
    if buffer * 1000 < downloadTime:
        RebufferTime += downloadTime - buffer * 1000
        buffer = 0.0
    else:
        buffer -= float(downloadTime) / 1000.0
    data = {'RebufferTime': RebufferTime, 'lastChunkSize': lastChunkSize, 'lastquality': lastquality, 'lastChunkStartTime': lastChunkStartTime, 'lastChunkFinishTime': lastChunkFinishTime, 'buffer': buffer, 'bandwidthEst': 0, 'lastRequest': lastRequest}
    data = json.dumps(data)
    r = requests.post(url=rl_server_address, data=data)

    # update according to rl_server's response that indicates quality
    lastChunkStartTime = lastChunkFinishTime
    lastRequest += 1
    lastquality = int(r.text)
    buffer += 4
